...
class TestConnectedAccountsAcl:
    ...
    def test_update_sharing_promotes_with_acl_in_one_call(
        ...
        """PRIVATE → SHARED + ACL grant atomically — the headline win that
        unblocks "share without re-auth"."""
        response = Mock()
        response.id = "ca_abc"
        response.status = "ACTIVE"
        response.success = True
    ...
    def test_update_sharing_demote_only(self, experimental, mock_client):
        """SHARED → PRIVATE with no ACL fields — backend clears the stored
        ACL atomically."""
    ...
    def test_update_sharing_promote_only_no_acl(self, experimental, mock_client):
        """PRIVATE → SHARED without ACL — credentials reused, ACL stays
        at the deny-by-default column state."""
